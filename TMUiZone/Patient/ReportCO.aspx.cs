...
public partial class ReportCO : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            try
            {
                if (Request.QueryString["No"] != null && Request.QueryString["No"] != string.Empty)
                {
                    ...
                    bindetails(Session["No"].ToString(), Service);
                }
                else
                {
                    MSG.Visible = false;
    ...
    public void bindetails(string Code,string Service)
    {
        SqlCommand cmd = new SqlCommand("[GetCovidDataReportIndivisual]", con);
        cmd.CommandType = CommandType.StoredProcedure;
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (dt.Rows[0]["Report Header Text1"].ToString().Contains("MICROBIOLOGY"))
            {
                ...
                if (dt.Rows[0]["Service Name"].ToString().Contains("COVID"))
                {

                    pnlRemark.Visible = true;
                    PnlRemark1.Visible = false;
                ...
                else if (dt.Rows[0]["Service Name"].ToString().Contains("RT-PCR"))
                {
                    pnlRemark.Visible = false;
                    PnlRemark1.Visible = true;
                    Label16.Text = "MICROBIOLOGY";
                    if (chkHeader.Checked == true || (Request.QueryString["No"] != null && Request.QueryString["No"] != string.Empty))
                    {
                        imglogo.Visible = true;
    ...
    public void BINDimage(string Service)
    {




        byte[] bytes = GetData("select [QR Code]  from [TMU$Hospital Report] where ([Lab No_]='" + Session["No"].ToString() + "' or [Registration No_]='" + Session["No"].ToString() + "') and [Service Name]='" + Service + "'").Rows[0]["QR Code"].ToString() == "" ? null : (byte[])GetData("select [QR Code]  from [TMU$Hospital Report] where ([Lab No_]='" + Session["No"].ToString() + "' or [Registration No_]='" + Session["No"].ToString() + "') and [Service Name]='" + Service + "'").Rows[0]["QR Code"];
        if (bytes != null)
        {
            string base64String = Convert.ToBase64String(bytes, 0, bytes.Length);
            imgQrCode.ImageUrl = "data:image/jpg;base64," + base64String;
        }
        byte[] bytes1 = GetData("exec GetSignatureNew '" + Session["No"].ToString() + "','" + Service + "'").Rows[0]["Signature"].ToString() == "" ? null : (byte[])GetData("exec GetSignatureNew '" + Session["No"].ToString() + "','" + Service + "'").Rows[0]["Signature"];
        if (bytes1 != null)
        {
