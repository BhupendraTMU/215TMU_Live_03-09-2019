...
public partial class StudentNoDues : System.Web.UI.Page
{
    ...
    public void binddate()
    {
        ...
        string strSQL = "select *,  FORMAT(CAST((SELECT top 1 [Creation Time] FROM [HRMSPortal].[dbo].[Tbl_StudentOutstanigNoDues] where [Enrollment No_]=a1.[Enrollement No] order by CAST([Creation Time] AS DATETIME) desc) AS DATETIME), 'dd/MM/yyyy') AS COEApprovaldate from Tbl_StudentNodues as a1 where [Enrollement No]='" + Session["enroll"].ToString() + "'";
        SqlDataAdapter da = new SqlDataAdapter(strSQL, con);
        DataTable dt = new DataTable();
        da.Fill(dt);
        con.Close();
    ...
    protected void lnkPay_Click(object sender, EventArgs e)
    {
        ...
        if (temp == 1)
        {
            ...
            string paytmURL = "https://secure.paytmpayments.com/order/process";
