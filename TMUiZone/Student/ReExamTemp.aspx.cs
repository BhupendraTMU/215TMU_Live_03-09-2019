...
public partial class Student_ReExamTemp : System.Web.UI.Page
{
    ...
    public void getExaminationDetail()
    {
        try
        {
            SqlCommand cmd = new SqlCommand("Sp_FetchExaminationDetails_ReapTemp", con);
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void btnCinvoice_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in GrdAppliedExamination.Rows)
        {
            ...
            if (chkRow.Checked)
            {
                ...
                DataTable dt = GetData(" select count(*) as C  from [TMU$Subject - COLLEGE] where [Code]='" + SubjectCode.Text + "' and [Academic Year]='" + Session["AcademicYear"].ToString() + "'");


                if (Convert.ToInt32(dt.Rows[0]["C"]) == 0)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Subject are not Available in Current Academic Year')", true);
                    return;
                }
            }
        ...
        else
        {
            ...
            da1.Fill(dt1);
            con.Close();
            if (Convert.ToInt32(dt1.Rows[0]["Flag"]) > 0 && 1 == 2)
            {
