...
public partial class Student_Events : System.Web.UI.Page
{
    ...
    public void bindGrid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select replace(convert(NVARCHAR, Date, 106), ' ', '-') as Date1,Event,E.Code,case E.College when '' then 'University' else 'College' end as Campus from TMU$Events E inner join [TMU$Student - COLLEGE] C on E.College=C.[Global Dimension 1 Code] or E.University='1' where No_='" + Session["uid"].ToString() + "' and [Date] >getdate()   order by Code Asc  ", con);
