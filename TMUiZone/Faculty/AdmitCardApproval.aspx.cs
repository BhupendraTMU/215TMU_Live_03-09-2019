...
public partial class Faculty_AdmitCardApproval : System.Web.UI.Page
{
    ...
    public void bindStudentExamDetails()
    {
        try
        {
            ...
            SqlCommand cmd = new SqlCommand("Sp_GetStudentExaminationDetailsFor_Admit_Card_Approval", con);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Select Course Code')", true);
                    return;
                }
                if (ddlSem.SelectedIndex == 0)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Select Semester')", true);
                    return;
                }
        ...
        try
        {
            ...
            if (ddlReaapear.SelectedValue=="1")
            {
                foreach (GridViewRow row in GrdExamList.Rows)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("ExaminationFormApproveFor_AdmitCard_aproval", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@CollegeCode", Session["GlobalDimension1Code"].ToString());
                    ...
                    {
                        cmd.Parameters.AddWithValue("@AdmitCardapproval", "0");

                    }

                    if (i > 0)
                    { ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Data Submitted')", true); }
                    else
                    {
            ...
            else
            {
                ...
                SqlCommand cmd11 = new SqlCommand("select COUNT([Subject Code]) as 'SDL' from   ( select distinct [Subject Code] from [TMU$Student Detainee List] where [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Subject Code] in (select Distinct [Subject Code] from [TMU$Student Optional Subject - COL] where  [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Course]='" + ddlCourse.SelectedValue + "' and (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "') union select Distinct [Subject Code] from [TMU$Student Subject - COLLEGE] where  [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Course]='" + ddlCourse.SelectedValue + "' and (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "')  and [Subject Code]!='') and   [Created Date]>'2024-04-15 00:00:00.000' ) TT  select COUNT( distinct [Subject Code]) as SSC from (select Distinct [Subject Code] from [TMU$Student Subject - COLLEGE] where [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Allow Subject]=1  and [Course]='" + ddlCourse.SelectedValue + "' and [Subject Code]!='' and  (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "') union select Distinct [Subject Code] from [TMU$Student Optional Subject - COL] where  [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Course]='" + ddlCourse.SelectedValue + "' and [Subject Code]!='' and (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "')) T", con);
               
                // SqlCommand cmd11 = new SqlCommand("select top 1 counted_leads as SDL from (Select count(distinct  [Subject Code]) as counted_leads, [Created Date] as count_date from [TMU$Student Detainee List] where [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Subject Code]!='' and [Course Code]='" + ddlCourse.SelectedValue + "' and  (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "')  group by [Created Date]) T order by count_date desc  select COUNT( distinct [Subject Code]) as SSC from (select Distinct [Subject Code] from [TMU$Student Subject - COLLEGE] where  [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Course]='" + ddlCourse.SelectedValue + "' and [Subject Code]!='' and  (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "') union  select Distinct [Subject Code] from [TMU$Student Optional Subject - COL] where  [Academic Year]='" + ddlAcademicYear.SelectedValue + "' and [Course]='" + ddlCourse.SelectedValue + "' and [Subject Code]!='' and (Semester='" + ddlSem.SelectedValue + "' or Year='" + ddlSem.SelectedValue + "')) T", con);
                con.Open();
                ...
                if (Convert.ToInt32(dt.Tables[0].Rows[0]["SDL"]) != Convert.ToInt32(dt.Tables[1].Rows[0]["SSC"]))
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Submit the Detainee list Subject Wise..')", true);
                    return;
                }
            ...
            foreach (GridViewRow row in GrdExamList.Rows)
            {
                CheckBox check = (CheckBox)row.FindControl("chkStudent");
                Label lblSemester = (Label)row.FindControl("lblSemester");
                Label lblCours = (Label)row.FindControl("lblCourse");

                SqlCommand cmd1 = new SqlCommand("sp_checkDetanieeListForAdmitcard", con);
                cmd1.CommandType = CommandType.StoredProcedure;
                cmd1.Parameters.AddWithValue("@CourseCode", lblCours.Text);
                ...
                cmd1.Parameters.AddWithValue("@SemYear", lblSemester.Text);
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Detainee Report Not  Submitted')", true);
