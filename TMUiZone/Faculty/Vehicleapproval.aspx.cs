...
public partial class Faculty_Vehicleapproval : System.Web.UI.Page
{
    ...
    public void SendSMSHODExam(string FromDate, string Ftime, string Todate,string name, string vehicle, string depart)
    {
        ...
        string mobilnoemp = "";
        string smsdata = "" +name+": A " +vehicle+" Reqd. at " +depart+ " "+Ftime+ ", " +FromDate+ " to "+Todate+".";
    
        // get no transport
        SqlDataReader dr = Portalcon.Show_TransportNo(Session["uid"].ToString(), tablenameemployeedata);
        dr.Read();
        if (dr.HasRows)
        {
    ...
    public void SendSMSUser(string FromDate, string Destination, string usemobile, string requisno)
    {
        ...
        string smsdata1 = "Your Transport Requisition " + requisno + " Journey Date " + FromDate + "Destination " + Destination + " has been rejected by " + Session["Fulname"].ToString() + " (HOD) " + txtRemarks.Text + ".";
    ...
    public void getgriddata()
    {
        try
        {
            SqlCommand cmd = new SqlCommand("Sp_getTransportVehicleapp", con1);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else if (Rblist.SelectedValue == "8")
                {
                    BtnSubmit.Visible = true;
                    BtnReject.Visible = false;
                }
                else if (Rblist.SelectedValue == "10")
                {
                    BtnSubmit.Visible = false;
                    BtnReject.Visible = false;
                }
        ...
        try
        {
            foreach (GridViewRow Row in GrdTransport.Rows)
            {
                ...
                if (checkRows.Checked == true)
                {
                    ...
                    con1.Open();
                    cmd.ExecuteNonQuery();
                    SendSMSHODExam(fromD.Value, Fromtime.Value, ToD.Value, Name.Value, Vehical.Value, Depart.Value);// for sms
                    con1.Close();
                }
            }
    ...
    protected void BtnRYes_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            foreach (GridViewRow Row in GrdTransport.Rows)
            {
                ...
                HiddenField UMoblie = (HiddenField)Row.FindControl("hdUmobile");
