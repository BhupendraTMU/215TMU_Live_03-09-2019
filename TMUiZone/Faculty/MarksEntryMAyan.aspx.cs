...
public partial class Faculty_MarksEntryMAyan : System.Web.UI.Page
{
    ...
    public void bindgrid()
    {
        try
        {
            ...
            if (dtS.Rows[0]["Marks Entry Faculty"] != "")
            {
                if (dtS.Rows[0]["Marks Entry Faculty"].ToString() != Session["uid"].ToString())
                {
    ...
    protected void btntblmarksshow_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // check Remaining methods marks entry

            //if ((hf_ExamMethod.Value.ToString().ToUpper()) == "ATT")
            //{

            //    SqlCommand cmdATT = new SqlCommand("sp_GetMethodForAttendance", con);
            //    cmdATT.CommandType = CommandType.StoredProcedure;

            //    cmdATT.Parameters.AddWithValue("@DocNo", grdlblDocumentNo.Value);
            //    cmdATT.Parameters.AddWithValue("@AcademicYear", drpAcademicYear.SelectedValue);
            //    cmdATT.Parameters.AddWithValue("@CollegeCode", Session["GlobalDimension1Code"].ToString());
            //    cmdATT.Parameters.AddWithValue("@ExamType", HFExamType.Value);

            //    cmdATT.Parameters.AddWithValue("@Semester", hf_SemYear.Value);
            //    cmdATT.Parameters.AddWithValue("@Subject", hf_SubjCode.Value);
            //    SqlDataAdapter daATT = new SqlDataAdapter(cmdATT);
            //    DataTable dtATT = new DataTable();
            //    con.Open();
            //    daATT.Fill(dtATT);
            //    if (dtATT.Rows[0]["Access"].ToString() == "0")
            //    {
            //        ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "alert", "callFeedbackMessage('Error', 'Please Input CT's Marks before Attendance.');", true);
            //        VisibleFalseTrue(false, false, true, false); LblInstruction.Visible = false;
            //        return;
            //    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    SaveOrSubmitData(hf_AcademicYear.Value, hf_Course.Value, hf_SemYear.Value, hf_SubjCode.Value, hf_SubjectType.Value, hf_ExamCriteria.Value, hf_ExamMethod.Value, hf_ExamGroup.Value, Convert.ToDecimal(hf_MaxMarks.Value), 1, gr);
                    getmethodcount();
                }
            }
    ...
    public bool ValidateBlankDataForSaveSubmit()
    {
        ...
        if (rdExternal.Checked = false)
        {
            ...
            //prentcount == Convert.ToInt32(lblpresentst.Text) ||
            {
                ...
                if ((AbCount == Convert.ToInt32(lblabsentst.Text) && UFMCount == Convert.ToInt32(LblUFMCount.Text)) || (hf_Classification.Value != "THEORY" && hf_Classification.Value != "THEORYPRACT"))
                {
                    return true;
                }
                else
    ...
    //public void SaveOrSubmitData(string AcademicYear, string CourseCode, string SemYear, string SubjectCode, string SubjectType, string ExamCriteria, string Method, string Group, decimal MaxMarks, int Reportstatus, GridView grdSelect)
    //{
    //    using (TransactionScope scope = new TransactionScope(TransactionScopeOption.RequiresNew))
    //    {
    //        try
    ...
    //                DataTable dt = new DataTable();
    //                dt.Clear();
    //                dt.Columns.Add("InernalMark");
    //                dt.Columns.Add("AttendanceType");
    ...
    //                                if (grdtxtMarks.Text != "")
    //                                {
    //                                    _Data["InernalMark"] = Convert.ToDecimal(grdtxtMarks.Text);
    //                                    _Data["AttendanceType"] = "1";

    //                                }
    //                                else
    //                                {
    //                                    _Data["InernalMark"] = Convert.ToDecimal("-1");
    //                                    _Data["AttendanceType"] = "1";
    ...
    //                                {
    //                                    _Data["InernalMark"] = Convert.ToDecimal("-1");
    //                                    _Data["AttendanceType"] = "2";
    //                                }
    //                                if (grdtxtMarks.Text == "DT" || grdtxtMarks.Text == "dt")
    //                                {
    //                                    _Data["InernalMark"] = Convert.ToDecimal("-1");
    //                                    _Data["AttendanceType"] = "3";
    //                                }
    //                                if (grdtxtMarks.Text == "UFM" || grdtxtMarks.Text == "ufm")
    //                                {
    //                                    _Data["InernalMark"] = Convert.ToDecimal("-1");
    //                                    _Data["AttendanceType"] = "4";
    //                                }
    ...
    //                if (Result == "SUCCESS")
    //                {
    //                    SqlCommand cmd1 = new SqlCommand("sp_InsertInternalMarksEntryAndAttendanceLineNew", con);//Insert_proc_MarksEntryLine1
    //                    cmd1.CommandType = CommandType.StoredProcedure;
    ...
    //                    cmd1.Parameters.AddWithValue("@Section", drpSection.SelectedValue);
    //                    cmd1.Parameters.AddWithValue("@AwardListStatus", Reportstatus);
    //                    cmd1.Parameters.AddWithValue("@InternalMarksTable", dt);
    //                    if (con.State != ConnectionState.Open)
    //                    {
    //                        con.Close();
    //                    }
    ...
    public void SaveOrSubmitData(string AcademicYear, string CourseCode, string SemYear, string SubjectCode, string SubjectType, string ExamCriteria, string Method, string Group, decimal MaxMarks, int Reportstatus, GridView grdSelect)
    {
        ...
        try
        {
            //DataTable MarksHeader = new DataTable();
            //MarksHeader.Clear();
            //MarksHeader.Columns.Add("CollegeCode");
            //MarksHeader.Columns.Add("AcadmicYear");
            //MarksHeader.Columns.Add("facultyCode");
            //MarksHeader.Columns.Add("CourseCode");
            //MarksHeader.Columns.Add("SemesterYear");
            //MarksHeader.Columns.Add("SubjectCode");
            //MarksHeader.Columns.Add("SubjectType");
            //MarksHeader.Columns.Add("ExamCriteria");
            //MarksHeader.Columns.Add("Status");
            //MarksHeader.Columns.Add("Method");
            //MarksHeader.Columns.Add("Group");
            //MarksHeader.Columns.Add("MaxMarks");
            //MarksHeader.Columns.Add("ExamType");
            //MarksHeader.Columns.Add("Section");
            //MarksHeader.Columns.Add("StudentGroup");
            //MarksHeader.Columns.Add("AwardListStatus");

            //DataRow _Data = MarksHeader.NewRow();

            //_Data["CollegeCode"] = Session["GlobalDimension1Code"].ToString();
            //_Data["AcadmicYear"] = AcademicYear;
            //_Data["facultyCode"] = Session["uid"].ToString();
            //_Data["CourseCode"] = CourseCode;
            //_Data["SemesterYear"] = SemYear;
            //_Data["SubjectCode"] = SubjectCode;
            //_Data["SubjectType"] = SubjectType;
            //_Data["ExamCriteria"] = ExamCriteria;
            //_Data["Status"] = hf_Status.Value;
            //_Data["Method"] = Method;
            //_Data["Group"] = Group;
            //_Data["MaxMarks"] = MaxMarks;
            //if (grdSelect.Rows.Count > 0)
            //{
            //    _Data["ExamType"] = hfExamType1.Value;
            //}
            //else
            //{
            //    _Data["ExamType"] = hf_ExamType.Value;
            //}
            //if (drpSection.SelectedIndex > 0)
            //{
            //    _Data["Section"] = drpSection.SelectedValue;
            //}
            //else
            //{
            //    _Data["Section"] = "";
            ...
            //    _Data["StudentGroup"] = ddlGroup.SelectedValue;
            //}
            //else
            //{
            //    _Data["StudentGroup"] = "";
            //}
            //_Data["AwardListStatus"] = Reportstatus;
            //MarksHeader.Rows.Add(_Data);
    ...
    protected void btnSavedata_Click(object sender, EventArgs e)
    {
        ...
        if (lblError.Text == "")
        {
            SaveOrSubmitData(hf_AcademicYear.Value, hf_Course.Value, hf_SemYear.Value, SubjectCode, SubjectType, DocumentNo, ExamMethod, ExamGroup, Convert.ToDecimal(MaxMarks), 0, gvR);
            ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "alert", "callFeedbackMessage('Success', 'Data saved Successfully');", true);
            bindgrid();
    ...
    protected void BtnSubmitdata_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            SaveOrSubmitData(hf_AcademicYear.Value, hf_Course.Value, hf_SemYear.Value, SubjectCode, SubjectType, DocumentNo, ExamMethod, ExamGroup, Convert.ToDecimal(MaxMarks), 1, gvR);
            ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "alert", "callFeedbackMessage('Success', 'Data Submitted Successfully');", true);
            bindgrid();
        }
    }
    ...
    public static string SavePivotData(string string_rowdata)
    {
        try
        {
            ...
            Faculty_MarksEntryMAyan obj = new Faculty_MarksEntryMAyan();
    ...
    protected void ddlSubject_TextChanged(object sender, EventArgs e)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (dt.Rows[0]["Subject Classification"].ToString() == "THEORY")
            {
                onetime.Visible = false;
            }
            else
