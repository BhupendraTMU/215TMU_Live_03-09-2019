...
public partial class Faculty_TimeSheet : System.Web.UI.Page
{
    ...
    public void BindTable(string FacultyCode)
    {
        ...
        {
            Command = "select * from [TMU$Time Table Generation - COL] where [Course Code]='" + drpCourseCode1.SelectedItem.ToString() + "'" + @"
                       and [Semester Code]='" + drpSemester.SelectedValue + "' and [Section Code] like '" + drpSection.SelectedValue + "%' " + qur + " " + @"
                       and [Day No] between  1 and 6 and [Hour No] between 1 and 10  and [Attendance Date] between dateadd(day,-(dbo.fun_GetDayNoMon2Sat(convert(date, getdate()))-1),convert(date,'" + txtFromDate.Text + "')) and dateadd(day,7,dateadd(day,-(dbo.fun_GetDayNoMon2Sat(convert(date, getdate()))),convert(date, getdate())))" + @"

Union select * from [TMU$Time Table Generation - COL] where [Course Code]='" + drpCourseCode1.SelectedItem.ToString() + "'" + @"
                       and [Year]='" + drpSemester.SelectedValue + "' and [Section Code] like '" + drpSection.SelectedValue + "%' " + qur + " " + @"
                       and [Day No] between  1 and 6 and [Hour No] between 1 and 10  and [Attendance Date] between dateadd(day,-(dbo.fun_GetDayNoMon2Sat(convert(date, getdate()))-1),convert(date, getdate())) and 
    ...
    {
        string proc = "Sp_GetCourseRoleWise_HOD_Role_ForTimeTable";
        if (Session["GlobalDimension1Code"].ToString() == "TMDC" || Session["GlobalDimension1Code"].ToString() == "TMMC")
        {
            proc = "Sp_GetCourseRoleWise_HOD_Role_ForTimeTableDM";
        }
    ...
    {
        SqlCommand cmd = new SqlCommand("select distinct([Faculty Code]),[Faculty Name] from [TMU$Course Wise Faculty] where [Course Code]='" + drpCourseCode1.SelectedItem.ToString() + "' and  [Portal ID]='1' order by [Faculty Name] asc", con);
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        DataTable dt = new DataTable();
        da.Fill(dt);
        ...
        drpFacultyCode.Items.Insert(0, new ListItem("-- Select --", "-- Select --"));
    ...
    protected void btnSeach_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "alert", "callFeedbackMessage('Error', 'Date can not be Blank.');", true);
        }
        else
        {
            BindTable(Session["uid"].ToString());
