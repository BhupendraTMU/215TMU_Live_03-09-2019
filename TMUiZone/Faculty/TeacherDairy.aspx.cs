...
public partial class Faculty_TeacherDairy : System.Web.UI.Page
{
            ...
            using (SqlCommand cmd = new SqlCommand("Pro_LessionPlanSummaryunitwise", con))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (var CourseCodeGroup in groupedCourseCode)
                    {
                        ...
                        lblInfo.Text = "<h3   style='font: bold; color: black; font-size: large'>" +
                                       "Program: " + Program + " &nbsp;&nbsp;&nbsp;&nbsp; " +
                                       "Course: " + CourseCode + "   &nbsp;&nbsp;&nbsp;&nbsp;       " +
    ...
    protected void Button69_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@Program", drpprogramlecturewisetheorycourse1.SelectedValue);
            ...
            using (SqlCommand cmd = new SqlCommand("Pro_LaboratoryWorkPlanLABCOURSE", con))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (var CourseCodeGroup in groupedCourseCode)
                    {
                        ...
                        Label lblInfo = new Label();
                        lblInfo.Text = "<h3 style='font: bold; color: black; font-size: large'>" +
                                       "Program: " + Program + " &nbsp &nbsp &nbsp &nbsp " +
                                       "Course: " + CourseCode + " &nbsp &nbsp &nbsp &nbsp " +
                                       "Course Name: " + CourseName + " &nbsp &nbsp &nbsp &nbsp " +
                                       "Semester: " + Semester1 + " &nbsp &nbsp &nbsp &nbsp " +
                                       "No of Credits: " + NoOfCredits + " &nbsp &nbsp &nbsp &nbsp " +
                                       "Total Teaching Hrs: " + TotalTeachingHrs +
                                       "</h3>";
    ...
    {
        SqlCommand cmd = new SqlCommand("HRMSPortal.dbo.Pro_bindStudentList", con1);
        cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        SqlCommand cmd = new SqlCommand("HRMSPortal.dbo.Pro_InsertStudentProjectsOngoing", con);
        cmd.CommandType = CommandType.StoredProcedure;
        ...
        cmd.Parameters.AddWithValue("@Programme", ddlProgram.SelectedValue);
        cmd.Parameters.AddWithValue("@UGPGLevel", ddlUGPG.SelectedItem.Text);
        cmd.Parameters.AddWithValue("@StudentsName", ddlStudentList.SelectedValue);
    ...
    //    SqlCommand cmd = new SqlCommand("[EDUCOLLEGELIVE-R2].[dbo].[proc_GetSubject]", con);
    //    cmd.CommandType = CommandType.StoredProcedure;
    //    cmd.Parameters.Add("@CourseCode", Drpprogram.SelectedValue);
    //    cmd.Parameters.Add("@SemesterYear", drpsemteacher.SelectedItem.Text);
    //    cmd.Parameters.Add("@FacultyCode", Session["uid"].ToString());
    ...
    protected void btnStudentproject_Click(object sender, EventArgs e)
    {
        Button btn = sender as Button;
        GridViewRow row = btn.NamingContainer as GridViewRow;
        string pk = grdstudentproject.DataKeys[row.RowIndex].Values[0].ToString();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["HRMSPortalConnectionString"].ToString());
        string strSQL = "Select * from StudentProjectsOngoing WHERE [ID]='" + pk + "'";
        SqlDataAdapter da = new SqlDataAdapter(strSQL, con);
        DataTable dt = new DataTable();
        da.Fill(dt);
        con.Close();
        ...
        cmd.Parameters.AddWithValue("@UserId", Session["uid"].ToString());
        cmd.Parameters.AddWithValue("@AcademicSession", dt.Rows[0]["AcademicYear"].ToString());
        cmd.Parameters.AddWithValue("@Semester", dt.Rows[0]["Semester1"].ToString());
        cmd.Parameters.AddWithValue("@Course", dt.Rows[0]["Programme"].ToString());
        ...
        txtidstudentongoing.Text= dt.Rows[0]["ID"].ToString();
        ddlUGPG.SelectedItem.Text = dt.Rows[0]["UGPGLevel"].ToString();
        ddlProgram.SelectedValue = dt.Rows[0]["Programme"].ToString();
        ddlSemester.SelectedItem.Text = dt.Rows[0]["Semester1"].ToString();
        ddlStudentList.SelectedValue = dt.Rows[0]["StudentsName"].ToString();
        txtProjectTopic.Text= dt.Rows[0]["ProjectTopic"].ToString();
        txtmonth1.Text = dt.Rows[0]["Month1"].ToString();
        txtmonth2.Text = dt.Rows[0]["Month2"].ToString();
        txtmonth3.Text = dt.Rows[0]["Month3"].ToString();
        txtmonths4.Text = dt.Rows[0]["Month4"].ToString();
        txtOverallEvaluation.Text = dt.Rows[0]["OverallEvaluation"].ToString();
        ModalPopupExtender9.Show();
