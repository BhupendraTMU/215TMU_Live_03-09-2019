...
public partial class Faculty_CertificateApproval : System.Web.UI.Page
{
    ...
    public void bindApprovalList()
    {
        try
        {
            SqlCommand cmd = new SqlCommand("[Pro_GetListForCertificateApproval]", con); //coment on usp_GetMarkEntrytable1 27-12-2017 by ashu sp_GetInternalMarksDataForApprovalByHOD
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        int result = 0;
        string StudentNo, Sem, Year, Course;
        if (grdCertificateApproval.Rows.Count > 0)
        {
            for (int i = 1; i <= grdCertificateApproval.Rows.Count; i++)
            {
                ...
                if (chk.Checked == true)
                {
                    ...
                    Label lblCourse = (Label)grdCertificateApproval.Rows[i - 1].FindControl("lblCourse");
                    Course = lblCourse.Text;
                    HiddenField hfStudentNo = (HiddenField)grdCertificateApproval.Rows[i - 1].FindControl("HfStNo");
                    StudentNo = hfStudentNo.Value;
                    HiddenField hfSSem = (HiddenField)grdCertificateApproval.Rows[i - 1].FindControl("HfSem");
                    Sem = hfSSem.Value;
                    HiddenField hfYear = (HiddenField)grdCertificateApproval.Rows[i - 1].FindControl("HfYear");
                    Year = hfYear.Value;
                    SqlCommand cmd;
                    cmd = new SqlCommand("[Sp_UpdateYearBackbyPrincipal]", con);
                    ...
                    if (result > 0)
                    {
                        ...
                        string navresult = nvp.YearBack(StudentNo,Course ,Sem,Year);
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Message", "alert('Approved Successfully')", true);
                        bindApprovalList();
                    }
                    else
                    {
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Message", "alert('Some thing went wrong..')", true);
                        return;
                    }
