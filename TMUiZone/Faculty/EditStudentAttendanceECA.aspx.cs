...
public partial class Faculty_EditStudentAttendanceECA : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                if ((Session["UserGroup"].ToString() == "PRINCIPAL" || Session["UserGroup"].ToString() == "DEAN") || Session["EventCo-Ordinator"] != "")
                {
    ...
    public void BinddlEventType1()
    {
        SqlCommand cmd = new SqlCommand("Sp_getEventattedance1", con); //ok
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.AddWithValue("@Reason", ddlEventType.SelectedValue);
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        DataTable dt = new DataTable();
        da.Fill(dt);
        ddlEventType1.DataSource = dt;
        ddlEventType1.DataTextField = "Details";
        ddlEventType1.DataValueField = "Value";
        ddlEventType1.Items.Insert(0, new ListItem("--Select--", ""));
        ddlEventType1.DataBind();
    }
    ...
    public static List<string> SearchCustomers(string prefixText, int count)
  {
    ...
    public class Customer
    {
        ...
        public string ddlEventType1 { get; set; }
    ...
    public static string presentstudentlist(DataTable dt1, string datefrom, string dateto, DataTable dtlecture)
    {
        ...
        if (dtstudentNo.Rows.Count > 0)
        {
            ...
            con.Close();
            return Returnmessage + PresentSt + " is present. So you can not mark ECA Attendance for the above Student.Please Correct your input and try again..!";
        }
        return "true";
    ...
    [System.Web.Script.Services.ScriptMethod()]
    [System.Web.Services.WebMethod]
    public static string Save(List<string> StudentNolist, string chkPresent, string EventType, string EventType1, string remarks, string DateFrom, string DateTo, List<int> Lecture)
    {
        ...
        string studentlist = "";
        if (EventType == "Extra Curricular Activities" || EventType == "Co-curricular Activities" || EventType == "OD" || EventType == "Cultural Events" || EventType == "Sports Events")
        {
        ...
        if (studentlist == "true" || studentlist=="")
        {
            if (FacultyCode != "")
            {
                try
                {
                    ...
                    cmd.CommandText = "Sp_UpdateAttendanceNew";
                    ...
                    cmd.Parameters.AddWithValue("@EventSubType", EventType1);
