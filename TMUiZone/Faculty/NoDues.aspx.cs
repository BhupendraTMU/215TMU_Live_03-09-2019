...
public partial class Faculty_NoDues : System.Web.UI.Page
{
    ...
    public void binddata()
    {
        ...
        string strSQL = "select No_,[First Name],[Father Name],[HOD Name],[HOD],[Global Dimension 1 Code],[Leave Date],[Job Title_Grade Desc],[Mobile Phone No_],[E-Mail],[Branch Name],Address,[Employment Date],[Employee Posting Group] from TMU$Employee where [No_]='" + Session["uid"].ToString() + "'";
        ...
        string val = dt.Rows[0]["Employee Posting Group"].ToString();
        if(val.Equals("NON-TEACH"))
        {
            SeedMoneyVal.Visible = false;
        }
        else
        {
            SeedMoneyVal.Visible = true;
        }
    ...
    public void alldept()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            DataRow[] dr25 = dt.Select("Particulars = 'Payroll'");
            if (dr25.Length > 0)
            {
                DropDownList22.SelectedItem.Text = dr25[0]["No_Dues"].ToString();
                txtremarkpayrollsection.Text = dr25[0]["Remark"].ToString();
            }
