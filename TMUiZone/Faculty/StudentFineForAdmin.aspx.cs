...
public partial class StudentFineForAdmin : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FINE"].ToString().Trim() == "FINE") { }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        dt.ToShortDateString();
            ...
            sdl.insertStudentDeciplineLine(hfStudentId.Value, txtCourse.Text, txtSemester.Text, txtSection.Text,
        ...
        if (drpActionTaken.SelectedItem.ToString() == "Fine")
        {
            ...
            if (datasourse == "172.0.1.108")
            {
                ...
                SqlDataAdapter da = new SqlDataAdapter(cmdNAV);
                da.Fill(dtNAV);
                ...
                vpWeb.StudentFine(hfStudentId.Value, Convert.ToDecimal(txtAmount.Text), txtRemarks.Text.Trim(), Convert.ToDateTime(txtDateCommited.Text));//Live
                }
            else
            {
               ...
               vpWeb.Credentials = new NetworkCredential("shubham\\administrator", "corp@123");//Test
               vpWeb.StudentFine(hfStudentId.Value, Convert.ToDecimal(txtAmount.Text), txtRemarks.Text.Trim(), Convert.ToDateTime(txtDateCommited.Text));//Test
            }
...
//                    and [Course Code] in (select distinct([Course Code]) from [TMU$Course Wise Faculty] where [Global Dimension 1 Code]=(select [Global Dimension 1 Code] from [TMU$Employee] where [No_]='" +FacultyCode+"'))";//17-10-2016
                    cmd.CommandText = "select [Student Name],[Enrollment No_],[No_] from [TMU$Student - COLLEGE] where  Upper([Enrollment No_]) like '" + prefixText.ToUpper() + "%'  OR Upper([Student Name]) like '" + prefixText.ToUpper() + "%' OR Upper([No_]) like '" + prefixText.ToUpper() + "%' " + @"
                   and [Student Status]=1 ";
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            customers.Add(sdr["Enrollment No_"].ToString() + "(" + sdr["Student Name"].ToString() + ")" + "(" + sdr["No_"].ToString() + ")");
    ...
    protected void txtStudentNo_TextChanged(object sender, EventArgs e)       
    {
        ...
        {
            dt = sdl.GetStudentDetailsForAdmin(txtStudentNo.Text);//sdl.GetStudentDetails(txtStudentNo.Text, CollegeCode); //open for all 10Jan2018 by Ashu
        }       
        
        if (dt.Rows.Count > 0)
        {           
            hfStudentId.Value = dt.Rows[0]["No_"].ToString();
            StudentNo = hfStudentId.Value;  
            ...
            txtCollege.Text = dt.Rows[0]["Global Dimension 1 Code"].ToString();
            btnSave.Enabled = true;
            bindGrid();
        }
        else
