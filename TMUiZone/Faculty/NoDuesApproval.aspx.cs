...
public partial class Faculty_NoDuesApproval : System.Web.UI.Page
{
    ...
    public void lnkbutton_Click1xx(object sender, EventArgs e)
    {
        ...
        {
            grdnodueslist.DataSource = null;
            grdnodueslist.DataBind();
            SqlCommand cmd = new SqlCommand("pro_nodueslistbyEmpNo", con1);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@UserId", Session["uid"].ToString());
            cmd.Parameters.AddWithValue("@EmpID", searchvalue.Text);
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            HiddenField hfStatus = (HiddenField)e.Row.FindControl("hfStatusxxx");
            if (hfStatus != null && hfStatus.Value.Equals("DONE"))
            {
    ...
    public void getdeptlibrary()
    {
        ...
        lblPayroll.Text = dtCL.Select("Particulars='Payroll Section'").CopyToDataTable().Rows[0]["Employee_Name"].ToString();
        txtPayroll.Text = dtCL.Select("Particulars='Payroll Section'").CopyToDataTable().Rows[0]["Employee_Code"].ToString();
        lblPayrollDesignation.Text = dtCL.Select("Particulars='Payroll Section'").CopyToDataTable().Rows[0]["Designation"].ToString();
        ...
        if (Session["uid"].ToString() == "TMU06837")
        {
        ...
        {
            drpPayroll.Enabled = true;
            txtPayrollRemark.Enabled = true;
            btnPayrollApproval.Enabled = true;
            btnPayrollReject.Enabled = true;
    ...
    public void hidehr()
    {
        ...
        string strSQL = "SELECT count(*) alldeptcount,e.[Employee Posting Group] as EmployeePostingGroup, n.Employee_Code FROM [EDUCOLLEGELIVE-R2].dbo.[TMU$Employee] e INNER JOIN [HRMSPortal].[dbo].Tbl_OutstandingNoDues n ON e.No_ COLLATE Latin1_General_CI_AI = n.Employee_Code COLLATE Latin1_General_CI_AI where No_Dues='Granted' and  Employee_Code ='" + txtemployeecode.Text + "' group by Employee_Code,[Employee Posting Group]";
        SqlDataAdapter da = new SqlDataAdapter(strSQL, con);
        DataTable dt = new DataTable();
        da.Fill(dt);
        if (dt.Rows.Count > 0)
        {
            if (Convert.ToInt32(dt.Rows[0]["alldeptcount"]) >= 20 && Convert.ToString(dt.Rows[0]["EmployeePostingGroup"]).Equals("NON-TEACH"))
            {
                divfinance.Visible = true;
                Button1.Visible = true;
            }
            else if (Convert.ToInt32(dt.Rows[0]["alldeptcount"]) >= 21)
            {
                divfinance.Visible = true;
                Button1.Visible = true;
    ...
    public void alldept()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            DataRow[] dr24 = dt.Select("Particulars = 'Payroll'");
            if (dr24.Length > 0)
            {
                drpPayroll.SelectedItem.Text = dr24[0]["No_Dues"].ToString();
                txtPayrollRemark.Text = dr24[0]["Remark"].ToString();
                txtPayroll1.Text = dr24[0]["ID"].ToString();

                if (drpPayroll.SelectedItem.Text == "Granted")
                {
                    btnPayrollApproval.ForeColor = System.Drawing.Color.White;
                    btnPayrollApproval.BackColor = System.Drawing.Color.Green;
                    drpPayroll.Enabled = false;
                    txtPayrollRemark.Enabled = false;
                    btnPayrollReject.Enabled = false;
                    btnPayrollApproval.Enabled = false;
                }
                else
                {
                    btnPayrollApproval.ForeColor = System.Drawing.Color.White;
                    btnPayrollApproval.BackColor = System.Drawing.Color.Red;
                    drpphddepartment.Enabled = true;
                    txtPayrollRemark.Enabled = true;
                    btnPayrollReject.Enabled = true;
                    btnPayrollApproval.Enabled = true;
    ...
    protected void btn_Hrstatus_Click(object sender, EventArgs e)
    {
        ...
        {
            btn_Hrstatus.Visible = true;
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Fill HR Resignation Date')", true);

            return;
        }
        //if (txtActdateofLeaving.Text == "")
        //{
        //    btn_Hrstatus.Visible = true;
        //    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Fill Actdate of Leaving')", true);
  ...
  public void sendmail()
    ...
    protected void btnPayrollApproval_Click(object sender, EventArgs e)
    {

        if (drpPayroll.SelectedItem.Text == "Select")
        {
            GridViewdata.Show();
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please Select Nodues ')", true);
            return;
        ...
        SqlCommand cmd = new SqlCommand("pro_OutstandingNoDues1", con);
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.AddWithValue("@ID", txtPayroll1.Text);
        cmd.Parameters.AddWithValue("@Particulars", Label65.Text);
        cmd.Parameters.AddWithValue("@No_Dues", drpPayroll.SelectedItem.Text);
        cmd.Parameters.AddWithValue("@Dept_Employee_Name", lblPayroll.Text);
        cmd.Parameters.AddWithValue("@Designation", lblPayrollDesignation.Text);
        cmd.Parameters.AddWithValue("@Remark", txtPayrollRemark.Text);
        cmd.Parameters.AddWithValue("@Employee_Code", txtemployeecode.Text);
        cmd.Parameters.AddWithValue("@Employee_Name", txtnameofemployee.Text);
        cmd.Parameters.AddWithValue("@Dept_Employee_Code", Session["uid"].ToString());
        cmd.Parameters.AddWithValue("@Status", "Submit");
    ...
    protected void btnPayrollReject_Click(object sender, EventArgs e)
    {
