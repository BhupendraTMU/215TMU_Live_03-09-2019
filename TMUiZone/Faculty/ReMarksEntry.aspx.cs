...
public partial class Faculty_MarksEntry : System.Web.UI.Page
{
    ...
    protected void rdOpen_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (rdOpen.Checked == true)
        {
            ...
            SqlCommand cmd = new SqlCommand("proc_GetSubjectFromsubjectreOpenelective", con);
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        SqlCommand cmd = new SqlCommand("proc_GetCourseFromReapear", con);
        cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        SqlCommand cmd = new SqlCommand("proc_GetSemesterFromCourseWiseFaculty_Reappear", con);
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.CommandTimeout = 1000000;
        cmd.Parameters.Add("@ID1", Session["uid"].ToString());
        cmd.Parameters.Add("@ID", drpCourse.SelectedValue);
        cmd.Parameters.Add("@AcademicYear1", drpAcademicYear.SelectedValue);
        DataTable dt = new DataTable();
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        con.Open();
        da.Fill(dt);
...

    {
        try
        {
            ...
            {
                SP = "sp_GetMarkEntryForFaculty_PivotReappear";
            }
            else
            {
                SP = "sp_GetMarkEntryForFaculty_Calendar_PNC2_TheoryReappear";
            }
            SqlCommand cmd = new SqlCommand(SP, con);//sp_GetMarkEntryForFaculty //sp_GetMarkEntryForFaculty_Calendar //comment on 02-05-2018
    ...
    protected void btntblmarksshow_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_GetStudentForInternalMarksEntry_PNC_Test_Internal2", con);//sp_GetStudentForInternalMarksEntry usp_GetMarkEntryStudent//sp_GetStudentForInternalMarksEntry
            ...
            if (dt.Rows[0]["Section"].ToString() == "0")
            {
    ...
    public bool ValidateBlankDataForSaveSubmit()
    {
        ...
        {
            if ((AbCount == Convert.ToInt32(lblabsentst.Text) && UFMCount == Convert.ToInt32(LblUFMCount.Text)) || hf_Classification.Value != "THEORY" || hf_ExamGroup.Value != "IOS")
            {
                return true;
            }
            else
    ...
    public void SaveOrSubmitData(string AcademicYear, string CourseCode, string SemYear, string SubjectCode, string SubjectType, string ExamCriteria, string Method
        , string Group, decimal MaxMarks, int Reportstatus, GridView grdSelect)
    {
    ...
    public void bindSubject()
    {
        try
        {
            SqlCommand cmd = new SqlCommand("sp_getSubjrctCodeFacultyWiseRe", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@FacultyCode", Session["uid"].ToString());
    ...
    protected void grdmarktable_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                SqlCommand cmd = new SqlCommand("sp_GetStudentForInternalMarksEntry_PNC_Test_Internal2", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@CollegeCode", Session["GlobalDimension1Code"].ToString());
                cmd.Parameters.AddWithValue("@AcadmicYear", drpAcademicYear.SelectedValue);
                cmd.Parameters.AddWithValue("@CourseCode", drpCourse.SelectedValue);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (dtpivot.Rows.Count > 0)
                    {
                        for (int k = 0; k < dtpivot.Rows.Count; k++)
                        {
                            ...
                            cmd1.Parameters.AddWithValue("@ExamCategory", "");
                            cmd1.Parameters.AddWithValue("@StudentGroup", "");
