...
public partial class Faculty_TransportVehicleapproval : System.Web.UI.Page
{
    ...
    public void SendSMSHODExam(string FromDate, string Ftime, string Todate, string Name, string mobil, string depar, string vehicl, string rplace)
    {
        ...
        string mobilnoemp = "";
        string smsdata = "" + Name+" " +mobil+" " +depar+": A " +vehicl+" Reqd. at "+rplace+"" + Ftime + ", " + FromDate + "to " + Todate +".";


        SqlDataReader dr = Portalcon.Show_Management(Session["uid"].ToString(), tablenameemployeedata);
        dr.Read();
        if (dr.HasRows)
        {
    ...
    public void SendSMSUser(string FromDate, string Destination, string usemobile, string requisno)
    {
        ...
        string smsdata1 = "Your Transport Requisition " + requisno + " Journey Date " + FromDate + "Destination " + Destination + " has been rejected by " + Session["Fulname"].ToString() + " (Transport) " + txtRemarks.Text + ".";
    ...
    public void getgriddata()
    {
        try
        {
            SqlCommand cmd = new SqlCommand("Sp_getTransportVehicleTAPP", con1);
            ...
            if (dt.Rows.Count > 0)
            {
                if (Rblist.SelectedValue == "4")
                {
                    BtnSubmit.Visible = true;
                    BtnReject.Visible = true;
                    BtnSendForA.Visible = true;
                }
                else if (Rblist.SelectedValue == "3")
                {
                    BtnSubmit.Visible = false;
                    BtnReject.Visible = true;
                    BtnSendForA.Visible = false;
                }
                ...
                else if (Rblist.SelectedValue == "9")
                {
                    BtnSubmit.Visible = false;
                    BtnReject.Visible = false;
                    BtnSendForA.Visible = false;
        ...
        try
        {
            foreach (GridViewRow Row in GrdTransport.Rows)
            {
                ...
                if (checkRows.Checked == true)
                {
                    ...
                    con1.Open();
                    cmd.ExecuteNonQuery();
                  // SendSMSHODExam(fromD.Value, Fromtime.Value, ToD.Value);// for sms// for sms
                    con1.Close();
                }
            }
    ...
    protected void BtnSendForA_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow Row in GrdTransport.Rows)
        {
            ...
            HiddenField Fromtime = (HiddenField)Row.FindControl("FTime");
            CheckBox checkRows = (CheckBox)Row.FindControl("chkDate");
            HiddenField Name = (HiddenField)Row.FindControl("HdNam");
            HiddenField Vehical = (HiddenField)Row.FindControl("hdVih");
            HiddenField Depart = (HiddenField)Row.FindControl("hdDepart");
            HiddenField Reqplace = (HiddenField)Row.FindControl("hdRqPlace");
            HiddenField Mobile = (HiddenField)Row.FindControl("hdMoble");
            if (checkRows.Checked == true)
            {
                SqlCommand cmd = new SqlCommand("sp_updateTransport", con1);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.ExecuteNonQuery();
                con1.Close();
                SendSMSHODExam(fromD.Value, Fromtime.Value, ToD.Value, Name.Value, Mobile.Value, Depart.Value, Vehical.Value, Reqplace.Value);// for sms// for sms
               
            }
        }
        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Send for approval successfully')", true);
        getgriddata();
    }
    protected void txtFromtDate_TextChanged(object sender, EventArgs e)
    {
        ...
        if (txtFromtDate.Text != "")
        {
            ...
            {
                txtFromtDate.Text = "";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('From Date should always be Greater than Current Date');", true);
