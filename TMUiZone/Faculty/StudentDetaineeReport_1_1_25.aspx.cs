...
public partial class StudentDetaineeReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (Session["UserGroup"].ToString() == "PRINCIPAL" || Session["UserGroup"].ToString() == "FACULTY" || Session["UserGroup"].ToString() == "DEAN")
            {
    ...
    public void GetSubjectList()
    {
        ...
        if(dt.Rows.Count>0)
        {
            txtPercentTo.Text= dt.Rows[0]["Dper"].ToString();
        }
    ...
    protected void btnDetanie_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in GrdDetenee.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                CheckBox chkRow = (row.Cells[0].FindControl("chkStudentUn") as CheckBox);             
                TextBox txtRemark = (row.Cells[0].FindControl("txtReason") as TextBox);
                if (chkRow.Checked == true && txtRemark.Text=="")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Please Enter Reason for Allow Detained Student.');", true);
                    return;
                }
            }
        }
        ...
        foreach (GridViewRow row in GrdDetenee.Rows)
        {
            ...
            CheckBox chkStudentUn = (CheckBox)row.FindControl("chkStudentUn");
            ...
            if (check.Checked == true)
            {
                ...
                else
                {
                    ...
                    cmd.Parameters.AddWithValue("@CHKUn", chkStudentUn.Checked.ToString());
    ...
    protected void chkStudentUn_CheckedChanged(object sender, EventArgs e)
    {

    }
    protected void chkAllUn_CheckedChanged(object sender, EventArgs e)
    {
        CheckBox checkBoxheader = (CheckBox)GrdDetenee.HeaderRow.FindControl("chkAllUn");
        foreach (GridViewRow Row in GrdDetenee.Rows)
        {
            CheckBox checkRows = (CheckBox)Row.FindControl("chkStudentUn");
            if (checkBoxheader.Checked == true)
            {
                checkRows.Checked = true;
            }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        GrdDetenee.Columns[7].Visible = false;
        GrdDetenee.Columns[8].Visible = false;
        GrdDetenee.Columns[9].Visible = false;
