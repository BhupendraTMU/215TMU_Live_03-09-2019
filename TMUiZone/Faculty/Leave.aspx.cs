...
public partial class LeaveApproval : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                //if (Session["uid"].ToString() == "TMU03642" || Session["uid"].ToString() == "TMU03944" || Session["uid"].ToString() == "TMU04160" || Session["uid"].ToString() == "TMU06074" || Session["uid"].ToString() == "TMVF0138" || Session["uid"].ToString() == "TMVF0139" || Session["uid"].ToString() == "TMVFOI40" || Session["uid"].ToString() == "TMU06202" || Session["uid"].ToString() == "TMU06221" || Session["uid"].ToString() == "TMU06248" || Session["uid"].ToString() == "TMU06691" || Session["uid"].ToString() == "TMU06566")
                //{
            ...
            if (Session["uid"].ToString() == "TMU03612")
            {
                btnSave.Visible = false;
            }
    ...
    public void Show_RecommendLeaveCount()
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("select count(*) as 'Final' from  [EDUCOLLEGELIVE-R2].dbo.[TMU$Pay Leave] where ([SA for Non-Teach Staff]='" + Session["uid"].ToString() + "' or [SA for Teaching Staff]='" + Session["uid"].ToString() + "')and [Leave Code] collate Latin1_General_100_CS_AS in (select Leave_Type from tble_Leave_Approval where  (HODUserid='" + Session["uid"].ToString() + "' or HODUserid1='" + Session["uid"].ToString() + "' or FinalApprovalId='" + Session["uid"].ToString() + "') and Company_Name='TMU' and (Status='Approved' )  and [Rejected Approval]='No'  )", con.Con);
        DataTable dt = new DataTable();
        da.Fill(dt);
    ...
    public void Valadation_CL()
    {
        if (ddLeaveTypen.SelectedItem.Text == "CL")
        {
            ...
            if (dr.HasRows)
            {
                ...
                DateTime PayrollProcessingMonthDate1 = Convert.ToDateTime(PayrollProcessingMonthDate);
                string PayrollProcessingMonthDate2 = PayrollProcessingMonthDate1.ToString("MM");
                //Decimal PayrollProcessingMonth = Convert.ToDecimal(12);
                Decimal PayrollProcessingMonth = Convert.ToDecimal(PayrollProcessingMonthDate2);
    ...
    public void LeaveApply()
    {
        ...
        else
        {
            ...
            if (flUpload.HasFile)
            {
                ...
                {
                    string sqlq = "insert into tble_Leave_Approval (From_Date,To_Date,Leave_Period,Reason,Address_Phone_No, No_Of_Days_Leave_Period,Total_Balance,HODUserid,HODUserid1,Company_Name,SerialNo,Create_Date,HR_Userid,Status,UName,UserID,Leave_Type,user_Emailid,HREmailid,HODEmailID,HRName,HODName,Arrangement,Half_Day_type_Code,Half_Day_type_Desc,PreLunch,PostLunch,FinalApprovalStatus,FinalApprovalId) values('" + fDate + "','" + tDate + "','" + leavetypedrop + "','" + txtReason.Text + "','" + txtPhoneNo.Text + "', '" + txtNoOfLeavePriod.Text + "','" + lblTotalBalance.Text + "','TMU04621','" + Session["HODLoginPage1"].ToString() + "','" + Session["Company"].ToString() + "','" + slno + "','" + System.DateTime.Now.ToString("dd/MM/yyyy") + "','" + hruid + "','Pending','" + Session["Fulname"].ToString() + "','" + Session["uid"].ToString() + "','" + ddLeaveTypen.Text + "','" + Session["CompanyEmail"].ToString() + "','" + Session["hr_email2"].ToString() + "','" + Session["hod_email2"].ToString() + "','" + Session["HRName"].ToString() + "','" + Session["hod_Name2"].ToString() + "','" + Arrangement + "','" + ddShiftType.SelectedValue.Trim() + "','" + ddShiftType.SelectedItem.Text.Trim() + "','" + prelunchdata + "','" + postlunchdata + "',0,'TMU04621' )";
    ...
    public void ValidateClCombined(string finalFromdate, string finaltodate)
    {
        ...
        for (int j = 0; j <= ds.Tables[0].Rows.Count - 1; j++)
        {
            ...
            else if (ddLeaveTypen.SelectedValue.Trim() != "CL")
            {
                ...
                {
                    txtNoOfLeavePriod.Text = "0";
                    ScriptManager.RegisterStartupScript(Page, this.GetType(), "myScript", "alert('CL can not combined with another leave except of CO');window.location ='Leave.aspx';", true);
                }
                else
                {
                    //code execute
    ...
    public void LeaveContinious()
    {
        try
        {
            ...
            if (Session["ShiftPattern"].ToString() == "0")
            {
                ...
                if (Session["ShiftDay"].ToString() == "1" && txtTodate.Text != "28 May 2023")
                {
                    weekdaycode = "Sunday";
                }
